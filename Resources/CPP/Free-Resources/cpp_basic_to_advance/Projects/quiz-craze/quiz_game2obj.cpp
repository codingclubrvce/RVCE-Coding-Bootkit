...
private:
    ...
    int correctAnswers;
    int wrongAnswers;
    int unattemptedAnswers;
    time_point<high_resolution_clock> start, stop;
    vector<Question> questions;

public:
    Quiz() : totalScore(0), correctAnswers(0), wrongAnswers(0), unattemptedAnswers(0) {}
    ...
    void runQuiz(const string &quizName, int quizTimeSeconds, bool enableNegativeMarking)
    {
        totalScore = 0;
        correctAnswers = 0;
        wrongAnswers = 0;
    ...
    void startQuiz()
    {
        ...
        do
        {
            ...
            case 1:
                ...
                {
                case 1:
                    questions = readQuestions("PHY.txt");
                    runQuiz("Physics", quizTimeLimit, enableNegativeMarking);
                    break;
                case 2:
                    questions = readQuestions("CHEM.txt");
                    runQuiz("Chemistry", quizTimeLimit, enableNegativeMarking);
                    break;
                case 3:
                    questions = readQuestions("C++.txt");
                    runQuiz("C++", quizTimeLimit, enableNegativeMarking);
                    break;
                case 4:
                    questions = readQuestions("CS.txt");
                    runQuiz("Computer Science", quizTimeLimit, enableNegativeMarking);
                    break;
                case 5:
                    questions = readQuestions("GK.txt");
                    runQuiz("General Knowledge", quizTimeLimit, enableNegativeMarking);
                    break;
                default:
